...
const Login: React.FC<LoginProps> = ({ onLogin }) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (!onLogin(username)) {
      setError('Invalid username. Try "supervisor" or "worker1".');
    }
  };
    ...
    <div className="min-h-screen bg-gray-100 flex flex-col justify-center items-center p-4">
      <div className="max-w-md w-full bg-white rounded-xl shadow-lg p-8 space-y-8">
        <div className="text-center">
            <h1 className="text-3xl font-bold text-primary">POA Manager</h1>
            <p className="mt-2 text-dark-gray">Sign in to your account</p>
        </div>
        <form onSubmit={handleSubmit} className="space-y-6">
            <div className="relative">
                ...
                <input
                    ...
                    placeholder="Username (e.g., supervisor, worker1, worker2)"
                    value={username}
